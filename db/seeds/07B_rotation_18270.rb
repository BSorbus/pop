...
############################################################################################
puts "... load from db/seeds/polisa_rotacja_18270.csv... start..."

File.open(File.join("db/seeds/log", 'polisa_rotacja_18270.log'), 'a+') do |f|
  f.puts "#####  07B_rotation_18270.rb  #####"
  f.puts "... load from db/seeds/polisa_rotacja_18270.csv... start..."
end 

CSV.foreach("db/seeds/polisa_rotacja_18270.csv", {  encoding: "WINDOWS-1250:UTF-8", 
                                            headers: true, 
  ...
  if @rotation.valid?
    ...
    File.open(File.join("db/seeds/log", 'polisa_rotacja_18270.error'), 'a+') do |f|
      f.puts "ERROR(s)"
