...
############################################################################################
puts "... load from db/seeds/firma_18270.csv... start..."

File.open(File.join("db/seeds/log", 'firma_18270.log'), 'a+') do |f|
  f.puts "#####  02B_companies_18270.rb  #####"
  f.puts "... load from db/seeds/firma_18270.csv... start..."
end 

CSV.foreach("db/seeds/firma_18270.csv", { encoding: "WINDOWS-1250:UTF-8", 
                                          headers: true, 
  ...
  if @company.valid?
    ...
    File.open(File.join("db/seeds/log", 'firma_18270.error'), 'a+') do |f|
      f.puts "ERROR(s)"
