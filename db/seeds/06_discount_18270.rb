...
############################################################################################
puts "... load from db/seeds/polisa_zz_18270.csv... start..."

File.open(File.join("db/seeds/log", 'polisa_zz_18270.log'), 'a+') do |f|
  f.puts "#####  06_discount_18270.rb  #####"
  f.puts "... load from db/seeds/polisa_zz_18270.csv... start..."
  f.puts "Discounts all: #{Discount.all.size}"
end 

CSV.foreach("db/seeds/polisa_zz_18270.csv", {  encoding: "WINDOWS-1250:UTF-8", 
                                            headers: true, 
  ...
  if @discount.valid?
    ...
    File.open(File.join("db/seeds/log", 'polisa_zz_18270.error'), 'a+') do |f|
      f.puts "ERROR(s)"
