...
############################################################################################
puts "... load from db/seeds/pop/files/polisa_h_00718.csv... start..."

File.open(File.join("db/seeds/pop/files/log", 'polisa_h_00718.log'), 'a+') do |f|
  f.puts "#####  04A_insurances_h_00718.rb  #####"
  f.puts "... load from db/seeds/pop/files/polisa_h_00718.csv... start..."
end 

CSV.foreach("db/seeds/pop/files/polisa_h_00718.csv", {  encoding: "WINDOWS-1250:UTF-8", 
                                            headers: true, 
  ...
  if InsuranceHistory.where(  insurance_id: id_with_offset( row[:id_polisa], DB00718_OFFSET_POLISA ),
    ...
    File.open(File.join("db/seeds/pop/files/log", 'polisa_h_00718.error'), 'a+') do |f|
