...
puts ""
puts "#####  04B_insurances_00718.rb  #####"

############################################################################################
puts "... load from db/seeds/pop/files/polisa_00718.csv... start..."

File.open(File.join("db/seeds/pop/files/log", 'polisa_00718.log'), 'a+') do |f|
  f.puts "#####  04B_insurances_00718.rb  #####"
  f.puts "... load from db/seeds/pop/files/polisa_00718.csv... start..."
end 

CSV.foreach("db/seeds/pop/files/polisa_00718.csv", {  encoding: "WINDOWS-1250:UTF-8", 
                                            headers: true, 
  ...
  @insurance = Insurance.create(  id:                   id_with_offset( row[:id_polisa], DB00718_OFFSET_POLISA ),
                                  number:               row[:numer], 
                                  ...
                                  created_at:           insurance_created_at_first_rec(id_with_offset(row[:id_polisa], DB00718_OFFSET_POLISA)),
  ...
  if @insurance.valid?
    ...
    File.open(File.join("db/seeds/pop/files/log", 'polisa_00718.error'), 'a+') do |f|
      f.puts "ERROR(s)"
...
File.open(File.join("db/seeds/pop/files/log", 'polisa_00718.log'), 'a+') do |f|
  ...
  f.puts "Insurances all where user=DB00718_USER_ID: #{Insurance.all.where(user: DB00718_USER_ID).size}"
  f.puts "#####  END ...load from 04B_insurances_00718.rb  #####"
end 
############################################################################################
...
puts "#####  END OF 04B_insurances_00718.rb  #####"
puts ""
