...
############################################################################################
puts "... load from db/seeds/pop/files/osoba_00718.csv... start..." 

File.open(File.join("db/seeds/pop/files/log", 'osoba_00718.log'), 'a+') do |f|
  f.puts "#####  03B_individuals_00718.rb  #####"
  f.puts "... load from db/seeds/pop/files/osoba_00718.csv... start..."
end 

CSV.foreach("db/seeds/pop/files/osoba_00718.csv", { encoding: "WINDOWS-1250:UTF-8", 
                                    headers: true, 
  ...
  @individual = Individual.create(id:                   id_with_offset( row[:id_osoba], DB00718_OFFSET_OSOBA ),
                                  first_name:           row[:imie], 
  ...
  if @individual.valid?
    ...
    File.open(File.join("db/seeds/pop/files/log", 'osoba_00718.error'), 'a+') do |f|
      f.puts "ERROR(s)"
