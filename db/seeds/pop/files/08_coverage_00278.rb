...
File.open(File.join("db/seeds/pop/files/log", 'polisa_rotacja_osoba_00278.log'), 'a+') do |f|
  f.puts "#####  08_coverage_00278.rb  #####"
  f.puts "... load from db/seeds/pop/files/polisa_rotacja_osoba_00278.csv... start..."
  f.puts "Coverages all: #{Coverage.all.size}"
end 

CSV.foreach("db/seeds/pop/files/polisa_rotacja_osoba_00278.csv", {  encoding: "WINDOWS-1250:UTF-8", 
                                            headers: true, 
  ...
  @coverage = Coverage.create(    id:           id_with_offset( row[:id_pro], DB00278_OFFSET_POLISA_ROTACJA_OSOBA ), 
  ...
  if @coverage.valid?
    ...
    File.open(File.join("db/seeds/pop/files/log", 'polisa_rotacja_osoba_00278.error'), 'a+') do |f|
      f.puts "ERROR(s)"
...
File.open(File.join("db/seeds/pop/files/log", 'polisa_rotacja_osoba_00278.log'), 'a+') do |f|
  f.puts "Coverages all: #{Coverage.all.size}"
  f.puts "#####  END ...load from 08_coverage_00278.rb  #####"
end 
############################################################################################
