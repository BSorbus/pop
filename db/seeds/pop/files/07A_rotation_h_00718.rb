...
puts ""
puts "#####  07A_rotation_h_00718.rb  #####"

############################################################################################
puts "... load from db/seeds/pop/files/polisa_rotacja_h_00718.csv... start..."

File.open(File.join("db/seeds/pop/files/log", 'polisa_rotacja_h_00718.log'), 'a+') do |f|
  f.puts "#####  07A_rotation_h_00718.rb  #####"
  f.puts "... load from db/seeds/pop/files/polisa_rotacja_h_00718.csv... start..."
  f.puts "RotationHistory all: #{RotationHistory.all.size}"
end 

CSV.foreach("db/seeds/pop/files/polisa_rotacja_h_00718.csv", {  encoding: "WINDOWS-1250:UTF-8", 
                                            headers: true, 
  ...
  if false
  #   RotationHistory.where( rotation_id:  id_with_offset( row[:id_polisa_rotacja], DB00718_OFFSET_POLISA_ROTACJA ), 
  #                          rotation_date:        row[:data_rotacji],
  #                          rotation_lock:        row[:blokada],
  #                          date_file_send:       nil,
  #                          note:                 row[:uwagi],
  #                          insurance_id:         id_with_offset( row[:polisa], DB00718_OFFSET_POLISA ) ).any?
  #  puts "Taki wpis już jest"
  #  #zapisz błąd do pliku
  #  File.open(File.join("db/seeds/pop/files/log", 'polisa_rotacja_h_00718.error'), 'a+') do |f|
  ...
  #  end 
    ...
    @rotation_h = RotationHistory.create( rotation_id:  id_with_offset( row[:id_polisa_rotacja], DB00718_OFFSET_POLISA_ROTACJA ), 
                                          rotation_date:        row[:data_rotacji],
                                          rotation_lock:        row[:blokada],
                                          date_file_send:       nil,
                                          note:                 row[:uwagi],
...
File.open(File.join("db/seeds/pop/files/log", 'polisa_rotacja_h_00718.log'), 'a+') do |f|
  f.puts "RotationHistory all: #{RotationHistory.all.size}"
  f.puts "#####  END ...load from 07A_rotation_h_00718.rb  #####"
end 
############################################################################################
...
puts "#####  END OF 07A_rotation_h_00718.rb  #####"
puts ""
