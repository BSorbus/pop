...
puts ""
puts "#####  05_group_18270.rb  #####"

############################################################################################
puts "... load from db/seeds/pop/files/polisa_grupa_18270.csv... start..."

File.open(File.join("db/seeds/pop/files/log", 'polisa_grupa_18270.log'), 'a+') do |f|
  f.puts "#####  05_group_18270.rb  #####"
  f.puts "... load from db/seeds/pop/files/polisa_grupa_18270.csv... start..."
  f.puts "Groups all: #{Group.all.size}"
end 

CSV.foreach("db/seeds/pop/files/polisa_grupa_18270.csv", {  encoding: "WINDOWS-1250:UTF-8", 
                                            headers: true, 
  ...
  @group = Group.create(          id:                   id_with_offset( row[:id_polisa_grupa], DB18270_OFFSET_POLISA_GRUPA ), 
                                  number:               row[:nr_grupy],
  ...
  if @group.valid?
    ...
    File.open(File.join("db/seeds/pop/files/log", 'polisa_grupa_18270.error'), 'a+') do |f|
      f.puts "ERROR(s)"
...
File.open(File.join("db/seeds/pop/files/log", 'polisa_grupa_18270.log'), 'a+') do |f|
  f.puts "Groups all: #{Group.all.size}"
  f.puts "#####  END ...load from 05_group_18270.rb  #####"
end 
############################################################################################
