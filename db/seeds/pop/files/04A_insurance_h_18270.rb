...
############################################################################################
puts "... load from db/seeds/pop/files/polisa_h_18270.csv... start..."

File.open(File.join("db/seeds/pop/files/log", 'polisa_h_18270.log'), 'a+') do |f|
  f.puts "#####  04A_insurances_h_18270.rb  #####"
  f.puts "... load from db/seeds/pop/files/polisa_h_18270.csv... start..."
end 

CSV.foreach("db/seeds/pop/files/polisa_h_18270.csv", {  encoding: "WINDOWS-1250:UTF-8", 
                                            headers: true, 
  ...
  if InsuranceHistory.where(  insurance_id: id_with_offset( row[:id_polisa], DB18270_OFFSET_POLISA ),
    ...
    File.open(File.join("db/seeds/pop/files/log", 'polisa_h_18270.error'), 'a+') do |f|
