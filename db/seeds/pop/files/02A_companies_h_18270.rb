...
############################################################################################
puts "... load from db/seeds/pop/files/firma_h_18270.csv... start..."

File.open(File.join("db/seeds/pop/files/log", 'firma_h_18270.log'), 'a+') do |f|
  f.puts "#####  02A_companies_h_18270.rb  #####"
  f.puts "... load from db/seeds/pop/files/firma_h_18270.csv... start..."
end 

CSV.foreach("db/seeds/pop/files/firma_h_18270.csv", { encoding: "WINDOWS-1250:UTF-8", 
                                    headers: true, 
  ...
  if CompanyHistory.where(company_id: id_with_offset(row[:id_firma], DB18270_OFFSET_FIRMA),
    ...
    File.open(File.join("db/seeds/pop/files/log", 'firma_h_18270.error'), 'a+') do |f|
