...
puts ""
puts "#####  06_discount_18270.rb  #####"

############################################################################################
puts "... load from db/seeds/pop/files/polisa_zz_18270.csv... start..."

File.open(File.join("db/seeds/pop/files/log", 'polisa_zz_18270.log'), 'a+') do |f|
  f.puts "#####  06_discount_18270.rb  #####"
  f.puts "... load from db/seeds/pop/files/polisa_zz_18270.csv... start..."
  f.puts "Discounts all: #{Discount.all.size}"
end 

CSV.foreach("db/seeds/pop/files/polisa_zz_18270.csv", {  encoding: "WINDOWS-1250:UTF-8", 
                                            headers: true, 
  ...
  @discount = Discount.create(    id:                   id_with_offset( row[:id_polisa_zz], DB18270_OFFSET_POLISA_ZZ ), 
                                  category:             row[:typ_zz],
  ...
  if @discount.valid?
    ...
    File.open(File.join("db/seeds/pop/files/log", 'polisa_zz_18270.error'), 'a+') do |f|
      f.puts "ERROR(s)"
...
File.open(File.join("db/seeds/pop/files/log", 'polisa_zz_18270.log'), 'a+') do |f|
  f.puts "Discounts all: #{Discount.all.size}"
  f.puts "#####  END ...load from 06_discount_18270.rb  #####"
end 
############################################################################################
...
puts "#####  END OF 06_discount_18270.rb  #####"
puts ""
