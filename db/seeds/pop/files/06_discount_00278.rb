...
puts ""
puts "#####  06_discount_00278.rb  #####"

############################################################################################
puts "... load from db/seeds/pop/files/polisa_zz_00278.csv... start..."

File.open(File.join("db/seeds/pop/files/log", 'polisa_zz_00278.log'), 'a+') do |f|
  f.puts "#####  06_discount_00278.rb  #####"
  f.puts "... load from db/seeds/pop/files/polisa_zz_00278.csv... start..."
  f.puts "Discounts all: #{Discount.all.size}"
end 

CSV.foreach("db/seeds/pop/files/polisa_zz_00278.csv", {  encoding: "WINDOWS-1250:UTF-8", 
                                            headers: true, 
  ...
  @discount = Discount.create(    id:                   id_with_offset( row[:id_polisa_zz], DB00278_OFFSET_POLISA_ZZ ), 
                                  category:             row[:typ_zz],
                                  description:          row[:opis_zz],
                                  discount_increase:    correct_decimal_separator(row[:zz_procent]),
                                  group_id:             id_with_offset( row[:polisa_grupa], DB00278_OFFSET_POLISA_GRUPA ))
  ...
  if @discount.valid?
    ...
    File.open(File.join("db/seeds/pop/files/log", 'polisa_zz_00278.error'), 'a+') do |f|
      f.puts "ERROR(s)"
...
File.open(File.join("db/seeds/pop/files/log", 'polisa_zz_00278.log'), 'a+') do |f|
  f.puts "Discounts all: #{Discount.all.size}"
  f.puts "#####  END ...load from 06_discount_00278.rb  #####"
end 
############################################################################################
...
#    @last_discount.destroy
    ...
    puts "#####  END OF 07_discount_00278.rb  #####"
    puts ""
