...
############################################################################################
puts "... load from db/seeds/pop/files/polisa_rotacja_00718.csv... start..."

File.open(File.join("db/seeds/pop/files/log", 'polisa_rotacja_00718.log'), 'a+') do |f|
  f.puts "#####  07B_rotation_00718.rb  #####"
  f.puts "... load from db/seeds/pop/files/polisa_rotacja_00718.csv... start..."
end 

CSV.foreach("db/seeds/pop/files/polisa_rotacja_00718.csv", {  encoding: "WINDOWS-1250:UTF-8", 
                                            headers: true, 
  ...
  if @rotation.valid?
    ...
    File.open(File.join("db/seeds/pop/files/log", 'polisa_rotacja_00718.error'), 'a+') do |f|
      f.puts "ERROR(s)"
