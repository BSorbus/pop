...
############################################################################################
puts "... load from db/seeds/pop/files/polisa_18270.csv... start..."

File.open(File.join("db/seeds/pop/files/log", 'polisa_18270.log'), 'a+') do |f|
  f.puts "#####  04B_insurances_18270.rb  #####"
  f.puts "... load from db/seeds/pop/files/polisa_18270.csv... start..."
end 

CSV.foreach("db/seeds/pop/files/polisa_18270.csv", {  encoding: "WINDOWS-1250:UTF-8", 
                                            headers: true, 
  ...
  if @insurance.valid?
    ...
    File.open(File.join("db/seeds/pop/files/log", 'polisa_18270.error'), 'a+') do |f|
      f.puts "ERROR(s)"
