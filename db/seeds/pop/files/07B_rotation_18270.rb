...
puts ""
puts "#####  07B_rotation_18270.rb  #####"

############################################################################################
puts "... load from db/seeds/pop/files/polisa_rotacja_18270.csv... start..."

File.open(File.join("db/seeds/pop/files/log", 'polisa_rotacja_18270.log'), 'a+') do |f|
  f.puts "#####  07B_rotation_18270.rb  #####"
  f.puts "... load from db/seeds/pop/files/polisa_rotacja_18270.csv... start..."
end 

CSV.foreach("db/seeds/pop/files/polisa_rotacja_18270.csv", {  encoding: "WINDOWS-1250:UTF-8", 
                                            headers: true, 
  ...
  @rotation = Rotation.create(    id:                   id_with_offset( row[:id_polisa_rotacja], DB18270_OFFSET_POLISA_ROTACJA ), 
                                  rotation_date:        row[:data_rotacji],
                                  rotation_lock:        row[:blokada],
                                  date_file_send:       nil,
                                  note:                 row[:uwagi],
                                  insurance_id:         id_with_offset( row[:polisa], DB18270_OFFSET_POLISA ),
                                  created_at:           rotation_created_at_first_rec( id_with_offset( row[:id_polisa_rotacja], DB18270_OFFSET_POLISA_ROTACJA ) ),
  ...
  if @rotation.valid?
    ...
    File.open(File.join("db/seeds/pop/files/log", 'polisa_rotacja_18270.error'), 'a+') do |f|
      f.puts "ERROR(s)"
...
File.open(File.join("db/seeds/pop/files/log", 'polisa_rotacja_18270.log'), 'a+') do |f|
  ...
  f.puts "#####  END ...load from 07B_rotation_18270.rb  #####"
end 
############################################################################################
...
puts "#####  END OF 07B_rotation_18270.rb  #####"
puts ""
