...
############################################################################################
puts "... load from db/seeds/pop/files/osoba_h_18270.csv... start..." 

File.open(File.join("db/seeds/pop/files/log", 'osoba_h_18270.log'), 'a+') do |f|
  f.puts "#####  03A_individuals_h_18270.rb  #####"
  f.puts "... load from db/seeds/pop/files/osoba_h_18270.csv... start..."
end 

CSV.foreach("db/seeds/pop/files/osoba_h_18270.csv", { encoding: "WINDOWS-1250:UTF-8", 
                                    headers: true, 
  ...
  if IndividualHistory.where( individual_id: id_with_offset( row[:id_osoba], DB18270_OFFSET_OSOBA ),
    ...
    File.open(File.join("db/seeds/pop/files/log", 'osoba_h_18270.error'), 'a+') do |f|
