...
puts ""
puts "#####  02A_companies_h_00278.rb  #####"

############################################################################################
puts "... load from db/seeds/pop/files/firma_h_00278.csv... start..."

File.open(File.join("db/seeds/pop/files/log", 'firma_h_00278.log'), 'a+') do |f|
  f.puts "#####  02A_companies_h_00278.rb  #####"
  f.puts "... load from db/seeds/pop/files/firma_h_00278.csv... start..."
end 

CSV.foreach("db/seeds/pop/files/firma_h_00278.csv", { encoding: "WINDOWS-1250:UTF-8", 
                                    headers: true, 
  ...
  if CompanyHistory.where(company_id: id_with_offset(row[:id_firma], DB00278_OFFSET_FIRMA),
                          short:                row[:skrot],
                          name:                 row[:nazwa],
    ...
    File.open(File.join("db/seeds/pop/files/log", 'firma_h_00278.error'), 'a+') do |f|
    ...
    @company_h = CompanyHistory.create( company_id:           id_with_offset( row[:id_firma], DB00278_OFFSET_FIRMA ), 
                                        short:                row[:skrot], 
                                        name:                 row[:nazwa],
                                        address_city:         row[:miejscowosc],
                                        address_street:       row[:ulica],
...
File.open(File.join("db/seeds/pop/files/log", 'firma_h_00278.log'), 'a+') do |f|
  ...
  f.puts "CompanyHistory all where user=DB00278_USER_ID: #{CompanyHistory.all.where(user_id: DB00278_USER_ID).size}"
  f.puts "#####  END ...load from 02A_companies_00278.rb  #####"
end 
############################################################################################
...
#
    puts "#####  END OF 02A_companies_h_00278.rb  #####"
    puts ""
