...
puts ""
puts "#####  05_group_00278.rb  #####"

############################################################################################
puts "... load from db/seeds/pop/files/polisa_grupa_00278.csv... start..."

File.open(File.join("db/seeds/pop/files/log", 'polisa_grupa_00278.log'), 'a+') do |f|
  f.puts "#####  05_group_00278.rb  #####"
  f.puts "... load from db/seeds/pop/files/polisa_grupa_00278.csv... start..."
  f.puts "Groups all: #{Group.all.size}"
end 

CSV.foreach("db/seeds/pop/files/polisa_grupa_00278.csv", {  encoding: "WINDOWS-1250:UTF-8", 
                                            headers: true, 
  ...
  @group = Group.create(          id:                   id_with_offset( row[:id_polisa_grupa], DB00278_OFFSET_POLISA_GRUPA ), 
                                  number:               row[:nr_grupy],
  ...
  if @group.valid?
    ...
    File.open(File.join("db/seeds/pop/files/log", 'polisa_grupa_00278.error'), 'a+') do |f|
      f.puts "ERROR(s)"
...
File.open(File.join("db/seeds/pop/files/log", 'polisa_grupa_00278.log'), 'a+') do |f|
  f.puts "Groups all: #{Group.all.size}"
  f.puts "#####  END ...load from 05_group_00278.rb  #####"
end 
############################################################################################
