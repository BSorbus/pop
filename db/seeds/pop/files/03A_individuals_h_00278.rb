...
puts ""
puts "#####  03A_individuals_h_00278.rb  #####"

############################################################################################
puts "... load from db/seeds/pop/files/osoba_h_00278.csv... start..." 

File.open(File.join("db/seeds/pop/files/log", 'osoba_h_00278.log'), 'a+') do |f|
  f.puts "#####  03A_individuals_h_00278.rb  #####"
  f.puts "... load from db/seeds/pop/files/osoba_h_00278.csv... start..."
end 

CSV.foreach("db/seeds/pop/files/osoba_h_00278.csv", { encoding: "WINDOWS-1250:UTF-8", 
                                    headers: true, 
  ...
  if IndividualHistory.where( individual_id: id_with_offset( row[:id_osoba], DB00278_OFFSET_OSOBA ),
                              first_name:           row[:imie], 
    ...
    File.open(File.join("db/seeds/pop/files/log", 'osoba_h_00278.error'), 'a+') do |f|
    ...
    @individual_h = IndividualHistory.create( individual_id:        id_with_offset( row[:id_osoba], DB00278_OFFSET_OSOBA ),
                                              first_name:           row[:imie], 
                                              last_name:            row[:nazwisko],
                                              address_city:         row[:miejscowosc],
                                              address_street:       row[:ulica],
...
File.open(File.join("db/seeds/pop/files/log", 'osoba_h_00278.log'), 'a+') do |f|
  ...
  f.puts "IndividualHistory all where user=DB00278_USER_ID: #{IndividualHistory.all.where(user_id: DB00278_USER_ID).size}"
  f.puts "#####  END ...load from 03A_individuals_h_00278.rb  #####"
end 
############################################################################################
...
#    @last_individual_h.destroy
    ...
    puts "#####  END OF 03A_individuals_h_00278.rb  #####"
    puts ""
