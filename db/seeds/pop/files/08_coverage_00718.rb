...
############################################################################################
puts "... load from db/seeds/pop/files/polisa_rotacja_osoba_00718.csv... start..."

File.open(File.join("db/seeds/pop/files/log", 'polisa_rotacja_osoba_00718.log'), 'a+') do |f|
  f.puts "#####  08_coverage_00718.rb  #####"
  f.puts "... load from db/seeds/pop/files/polisa_rotacja_osoba_00718.csv... start..."
  f.puts "Coverages all: #{Coverage.all.size}"
end 

CSV.foreach("db/seeds/pop/files/polisa_rotacja_osoba_00718.csv", {  encoding: "WINDOWS-1250:UTF-8", 
                                            headers: true, 
  ...
  if @coverage.valid?
    ...
    File.open(File.join("db/seeds/pop/files/log", 'polisa_rotacja_osoba_00718.error'), 'a+') do |f|
      f.puts "ERROR(s)"
