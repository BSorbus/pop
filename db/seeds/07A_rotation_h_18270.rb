...
############################################################################################
puts "... load from db/seeds/polisa_rotacja_h_18270.csv... start..."

File.open(File.join("db/seeds/log", 'polisa_rotacja_h_18270.log'), 'a+') do |f|
  f.puts "#####  07A_rotation_h_18270.rb  #####"
  f.puts "... load from db/seeds/polisa_rotacja_h_18270.csv... start..."
  f.puts "RotationHistory all: #{RotationHistory.all.size}"
end 

CSV.foreach("db/seeds/polisa_rotacja_h_18270.csv", {  encoding: "WINDOWS-1250:UTF-8", 
                                            headers: true, 
  ...
  #  File.open(File.join("db/seeds/log", 'polisa_rotacja_h_18270.error'), 'a+') do |f|
