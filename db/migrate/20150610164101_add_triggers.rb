class AddTriggers < ActiveRecord::Migration
  def self.up
    ...
    ###### families ######
    execute(
      "CREATE OR REPLACE FUNCTION family_save_his()
        RETURNS trigger AS
      $BODY$BEGIN
      insert into family_histories (
        ...
        updated_at ) values (
          ...
          NEW.applies_to,
          NEW.pay,
          NEW.protection_variant,
          NEW.assurance,
          NEW.assurance_component,
          NEW.family_lock,
          NEW.note,
          NEW.company_id,
          NEW.user_id,
    ...
    execute(
      "CREATE TRIGGER tr_family_save_his
        AFTER INSERT OR UPDATE
        ON families
        FOR EACH ROW
        EXECUTE PROCEDURE family_save_his();")
    ###### ./insurances ######


    ###### rotations ######
    execute(
      "CREATE OR REPLACE FUNCTION family_rotation_save_his()
        RETURNS trigger AS
      $BODY$BEGIN
      insert into family_rotation_histories (
        family_rotation_id,
        rotation_date,
        rotation_lock,
        date_file_send,
    ...
    execute(
      "CREATE TRIGGER tr_family_rotation_save_his
        AFTER INSERT OR UPDATE
        ON family_rotations
        FOR EACH ROW
        EXECUTE PROCEDURE family_rotation_save_his();")
    ###### ./rotations ######
  ...
  def self.down
    ...
    ###### families ######
    execute("DROP TRIGGER IF EXISTS tr_family_save_his ON families;")
    execute("DROP FUNCTION IF EXISTS family_save_his() CASCADE;")
    ###### ./insurances ######

    ###### rotations ######
    execute("DROP TRIGGER IF EXISTS tr_family_rotation_save_his ON family_rotations;")
    execute("DROP FUNCTION IF EXISTS family_rotation_save_his() CASCADE;")
    ###### ./rotations ######
  end
