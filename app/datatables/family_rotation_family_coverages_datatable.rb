class FamilyRotationFamilyCoveragesDatatable < AjaxDatatablesRails::Base
  ...
  def data
    ...
    records.map do |record|
      [
        ...
        record.note,
        link_to(' ', @view.edit_family_coverage_path(record.id, back_url: @view.family_family_rotation_path(record.family_rotation.family, record.family_rotation)), 
  ...
  def get_raw_records
    FamilyCoverage.joins({family_rotation: [:family]}).includes(:insured, :payer).by_family_rotation(options[:only_for_family_rotation_id]).references(:family_rotation, :insured, :payer).all
  end
