class InsuredFamilyCoveragesDatatable < AjaxDatatablesRails::Base
  ...
  def get_raw_records
    FamilyCoverage.joins({family_rotation: [:family]}).includes(:insured, :payer).by_insured(options[:only_for_insured_id]).references(:family_rotation, :insured, :payer).all
  end
