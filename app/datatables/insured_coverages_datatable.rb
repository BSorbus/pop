class InsuredCoveragesDatatable < AjaxDatatablesRails::Base
  ...
  def data
    ...
    records.map do |record|
      [
        ...
        #link_to(" #{record.rotation.insurance.number}", @view.insurance_path(record.rotation.insurance),
        #            class: record.rotation.insurance.insurance_lock? ? 'glyphicon glyphicon-lock' : ' '),
        #link_to(" #{record.rotation.rotation_date}", @view.insurance_rotation_path(record.rotation.insurance, record.rotation),
  ...
  def get_raw_records
    Coverage.joins({rotation: [:insurance]}, :group).includes(:insured, :payer).by_insured(options[:only_for_insured_id]).references(:rotation, :group, :insured, :payer).all
  end
